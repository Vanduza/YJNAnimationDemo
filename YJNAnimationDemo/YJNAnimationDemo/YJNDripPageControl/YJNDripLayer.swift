...
import UIKit
enum YJNMovingDirect {
    case left
    case right
}

fileprivate let outsideRectSize: CGFloat = 90

class YJNDripLayer: CALayer {
    ...
    private var movingDirect: YJNMovingDirect!
    ...
    override init(layer: Any) {
        ...
        if let layer = layer as? YJNDripLayer {
    ...
    override func draw(in ctx: CGContext) {
        ...
        let pointB = CGPoint(x: movingDirect == YJNMovingDirect.left ? rectCenter.x + outsideRect.size.width/2 : rectCenter.x + outsideRect.size.width/2 + movedDistance*2, y: rectCenter.y)
        let pointC = CGPoint(x: rectCenter.x, y: rectCenter.y + outsideRect.size.height/2 - movedDistance)
        let pointD = CGPoint(x: movingDirect == YJNMovingDirect.left ? outsideRect.origin.x - movedDistance*2 : outsideRect.origin.x, y: rectCenter.y)
        
        let c1 = CGPoint(x: pointA.x + offset, y: pointA.y)
        let c2 = CGPoint(x: pointB.x, y: movingDirect == .left ? pointB.y - offset : pointB.y - offset + movedDistance)
        
        let c3 = CGPoint(x: pointB.x, y: movingDirect == .left ? pointB.y + offset : pointB.y + offset - movedDistance)
        let c4 = CGPoint(x: pointC.x + offset, y: pointC.y)
        
        let c5 = CGPoint(x: pointC.x - offset, y: pointC.y)
        let c6 = CGPoint(x: pointD.x, y: movingDirect == .left ? pointD.y + offset - movedDistance : pointD.y + offset)
        
        let c7 = CGPoint(x: pointD.x, y: movingDirect == .left ? pointD.y - offset + movedDistance : pointD.y - offset)
        ...
        ctx.setLineDash(phase: 0, lengths: [CGFloat(5.0),CGFloat(5.0)])
        ctx.strokePath()
        ...
        ctx.setLineDash(phase: 0, lengths: [CGFloat(5),CGFloat(5)])
